...
FOLLOWUP_QUERY_PROMPT = """你正在使用搜索工具通过迭代搜索数据库来回答主要查询。根据以下中间查询和答案，生成一个新的简单后续问题，以帮助回答主要查询。当之前的答案没有帮助时，你可以重新表述或分解主要查询。只提出简单的后续问题，因为搜索工具可能无法理解复杂的问题。
...
请用中文回答一个简单的后续问题，以帮助回答主要查询，不要解释自己或输出其他内容。
"""

INTERMEDIATE_ANSWER_PROMPT = """根据以下文档，为查询生成适当的答案。请勿编造任何信息，仅使用提供的文档生成答案。如果文档中没有有用的信息，请回答“未找到相关信息”。
...
class ChainOfRAG(RAGAgent):
    ...
    def _retrieve_and_answer(self, query: str) -> Tuple[str, List[RetrievalResult], int]:
        ...
        text = re.sub(r'<think>.*?</think>', '', query, flags=re.DOTALL)
        text = re.sub(r'<think>', '', text)
        query = re.sub(r'</think>', '', text)
        query = query.strip()
